...
class Tloader extends React.Component {
  ...
  touchMove = (e) => {
    ...
    if (distance > 0 && scrollTop <= 0) {
      ...
      if (pullHeight)
        ...
        this.setState({
          loaderState:
            pullHeight > distanceToRefresh ? STATS.enough : STATS.pulling,
          pullHeight,
        });
    }
  };
  ...
  canRefresh() {
    ...
    return (
      onRefresh && [STATS.refreshing, STATS.loading].indexOf(loaderState) < 0
    );
  }
  ...
  render() {
    const { children, className, hasMore, initializing, swipeHandlers } =
      this.props;
    const { loaderState, pullHeight, progressed } = this.state;

    const footer = hasMore ? (
      <div className={styles['tloader-footer']}>
        <div className={styles['tloader-btn']} onClick={this.loadMore} />
      ...
      children !== 0 && (
        <div className={styles['tloader-footer']}>
          <div className={styles['tloader-no-more']}>
    ...
    if (!progressed) {
      if (initializing > 0)
        progressClassName += ` ${styles['tloader-progress']}`;
      if (initializing > 1) progressClassName += ` ${styles['ed']}`;
    }

    const scrollClassName = `${styles.tloader} ${
      styles[`state-${loaderState}`]
    ...
    return (
      <div
        ...
        ref={(el) => {
          this.panel = el;
          swipeHandlers?.ref && swipeHandlers?.ref(el);
        }}
      ...
      >
        <div className={styles['tloader-symbol']}>
          <div className={styles['tloader-msg']}>
            <i />
          </div>
          <div className={styles['tloader-loading']}>
            <i className={styles['ui-loading']} />
        ...
        <div className={styles['tloader-body']} style={style}>
          {children !== 0 && children}
        </div>
        {footer}
      </div>
